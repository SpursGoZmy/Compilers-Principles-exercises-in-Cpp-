...
//        if(kind==FunK||kind==Var_DeclK||kind==Array_DeclK||kind==ParamsK||kind==Int_ParamK||kind==Array_ParamK||kind==CompK||kind==Selection_StmtK
//           ||kind==Iteration_StmtK||kind==Return_StmtK||kind==AssignK||kind==OpK||kind=Array_ElemK||kind==CallK||kind==ArgsK||)
//        {
//            t->scope=copyString(scope);
//        }
