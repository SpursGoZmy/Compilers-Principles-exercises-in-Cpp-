...
TreeNode* declaration()
{
    ...
    if((p!=NULL)&&(token==ID))  //前面没有出错，而且紧跟着第二个token为ID
    {
        ...
        if(token==LPAREN)    //下一个token为 ( ,说明该声明为函数声明
        {
            ...
            char * function_name=copyString(q->attr.name);   //修改作用域

            sprintf(scope, "%s", function_name);


            Scope_List->scope_names[Scope_List->current_num]=copyString(scope);
            Scope_List->current_num++;

            t->child[2]=params();
            //TreeNode* m=t->child[2];
            match(RPAREN);
            t->child[3]=compound_stmt();

            char * s="global";
            sprintf(scope,"%s",s);   //一个函数声明识别完成，将作用域修改回global
            if_count=0;
            while_count=0;
            local_block_count=0;
...
TreeNode* param()       //构建单个Int参数或者数组参数的结点
{
   ...
   // q->scope=copyString(scope);   //函数形参声明要设置作用域
...
TreeNode* statement()     //进一步判断statement的类型
{
    ...
    case IF:
        {
        char *str="if_statement";   //更新作用域
        char  if_index[256];        //用来存储转换后的if_count
        char *old_scope=copyString(scope);
        sprintf(scope,"%s-%s%s",old_scope,str,itoa(if_count,if_index,10));
        if_count++;
        Scope_List->scope_names[Scope_List->current_num]=copyString(scope);
        Scope_List->current_num++;
        ...
        sprintf(scope,"%s",old_scope);

        break;
        }
    case WHILE:
        {
        char *str= "while_statement";    //更新作用域
        char *old_scope=copyString(scope);
        char while_index[256];
        sprintf(scope,"%s-%s%s",old_scope,str,itoa(while_count,while_index,10));
        while_count++;
        Scope_List->scope_names[Scope_List->current_num]=copyString(scope);
        Scope_List->current_num++;
    ...
    case LBBRACKET:
        ...
        char *str="local_block";
        char *old_scope=copyString(scope);
        char local_block_index[256];
        sprintf(scope,"%s-%s%s",old_scope,str,itoa(local_block_count,local_block_index,10));
        local_block_count++;
        Scope_List->scope_names[Scope_List->current_num]=copyString(scope);
        Scope_List->current_num++;
